...
func NewCustomModel(schema func() (*ytypes.Schema, error), modelData []*gnmipb.ModelData,
	...
	if m.StateSync == nil {
		if glog.V(10) {
			glog.Infof("StateSync interface is not installed.")
			glog.Infof("The model starts without StateSync")
		}
	}
	m.initStateSync()
	return m, nil
...
func (m *Model) UpdateCreate(path string, value string) error {
	entry := m.FindSchemaByXPath(path)
	if entry == nil {
		if glog.V(10) {
			glog.Errorf("model.create:: unknwon path in %s", path)
		}
		return nil
	}
...
func (m *Model) UpdateReplace(path string, value string) error {
	if entry := m.FindSchemaByXPath(path); entry == nil {
		if glog.V(10) {
			glog.Errorf("model.replace:: unknwon path in %s", path)
		}
		return nil
	}
	gpath, err := xpath.ToGNMIPath(path)
...
func (m *Model) UpdateDelete(path string) error {
	...
	if entry == nil {
		if glog.V(10) {
			glog.Errorf("model.delete:: unknwon path in %s", path)
		}
		return nil
	}
